...
class WorkoutManagerController {
  ...
  public async getUserWorkoutById(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (error instanceof Error && error.message === "Workout record not found") {
  ...
  public async getUserActiveWorkout(req: Request, res: Response): Promise<void> {
    try {
      const activeWorkout = await workoutManager.getUserActiveWorkout(req.user._id);
      CreateApiSuccess(activeWorkout, 200, res);
    } catch (error) {
      if (error instanceof Error && error.message === "No active workout found") {
        return CreateApiError("No active workout found", 404, res);
      }
      CreateApiError("Failed to fetch active workout", 500, res);
    }
  }

  public async postUserActiveWorkout(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "Invalid userId or workoutId") {
